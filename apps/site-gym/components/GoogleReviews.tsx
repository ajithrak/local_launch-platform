        ...
        <div className="flex flex-col items-center gap-3 sm:flex-row">
          ...
          <div>
            <div className="flex items-center justify-center gap-2 sm:justify-start">
              <span style={{ fontFamily: 'var(--font-display)', fontSize: 28, color: 'var(--color-foreground)' }}>
              ...
              <div className="flex gap-0.5">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star
                    key={i}
                    size={16}
                    fill={i < fullStars ? '#FBBC05' : 'none'}
                    color={i < fullStars ? '#FBBC05' : 'var(--color-border)'}
                  />
                ))}
              </div>
            </div>
            <p className="text-sm" style={{ fontFamily: 'var(--font-body)', color: 'var(--color-muted)' }}>
              Based on {reviews.reviewCount.toLocaleString()} Google reviews
            </p>
          </div>
        </div>
        <a
